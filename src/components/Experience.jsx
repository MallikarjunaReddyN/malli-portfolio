  ...
  return (
    <section id="experience" className="py-10 relative">
      <div className="mt-8 text-gray-100 text-center">
        ...
        <span className="text-[#EE3F37]">My</span> Experience
        </h3>
        <p className="text-gray-400 mt-3 text-lg">My Work</p>
        <div className="flex items-center justify-center mt-12 gap-10 flex-wrap">
        {companies.map((company, i) => (
            <div key={i} className={`min-w-[12rem] max-w-[20rem] p-4 ${company.from_to.toLowerCase().includes("present") ? "bg-[#EE3F37]" : "bg-slate-700" } rounded-xl`}>
            <img src={company.img} alt="" className="rounded-lg w-[250px] h-[180px]" />
            <h3 className="text-xl my-4 font-bold">{company.name}</h3>
            <h3 className={`font-bold ${company.from_to.toLowerCase().includes("present") ? "text-white" : "text-slate-400" } my-4`}>{company.designation}</h3>
            <h3 className={`font-bold ${company.from_to.toLowerCase().includes("present") ? "text-white" : "text-slate-400" } my-4`}>{company.from_to}</h3>
            <div className="flex gap-3 pl-[75px]">
            { company.social_media?.map((media, i) => (
              <a key={i}
                href={media.link}
                target="_blank"
                className={`${company.from_to.toLowerCase().includes("present") ? "text-slate-300" : "text-slate-400" } text-2xl hover:text-white cursor-pointer`}
              >
